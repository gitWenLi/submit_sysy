...
<template>
  <div class="process_agreen">
    ...
    width="100%">
      ...
      label="操作">
      <template v-slot="{ row }">
        <el-button type="primary" round plain size="small" @click="goDetail(row)" ><i class="el-icon-edit-outline"></i>审批</el-button>
      </template>
      </el-table-column>
    </el-table>
    <!-- 分页 -->
...
export default {
  ...
  // 分页
    ...
    // 详情
    goDetail (row) {
      this.$router.push({ path: '/processdetail', query: { id: row._id, state: row.auditResult } })
      // console.log(row._id)
    },
    // 获取表格数据
    async getTableData () {
